...
public class EditorCameraSpeed : EditorWindow
{
    ...
    public static void OnSceneDelegate(SceneView sceneView)
    {
        ...
        var locked_view_tool_field =
            (FieldInfo) tools_type.GetField("s_LockedViewTool", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        /*var scene_view_flyspeed_type = */
        scene_view_assembly.GetType("UnityEditor.SceneViewMotion");

        var flyspeed_field =
            (FieldInfo) scene_view_motion_type.GetField("s_FlySpeed", BindingFlags.NonPublic | BindingFlags.Static);
