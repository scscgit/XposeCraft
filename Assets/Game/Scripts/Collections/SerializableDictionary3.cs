...
namespace XposeCraft.Collections
{
    ...
    /// To use, inherit this generic class to a non-generic version (no need to be public) and annotate [Serializable].
    ...
    public class SerializableDictionary3<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        public SerializableDictionary3()
            : this(0, null)
        {
        }

        public SerializableDictionary3(int capacity)
            : this(capacity, null)
        {
        }

        public SerializableDictionary3(IEqualityComparer<TKey> comparer)
            : this(0, comparer)
        {
        }

        public SerializableDictionary3(int capacity, IEqualityComparer<TKey> comparer)
        {
            if (capacity < 0)
        ...
        public SerializableDictionary3(IDictionary<TKey, TValue> dictionary)
            : this(dictionary, null)
        {
        }

        public SerializableDictionary3(IDictionary<TKey, TValue> dictionary, IEqualityComparer<TKey> comparer)
        ...
        {
            private readonly SerializableDictionary3<TKey, TValue> _dictionary3;
            ...
            internal Enumerator(SerializableDictionary3<TKey, TValue> dictionary3)
            {
                _dictionary3 = dictionary3;
                _version = dictionary3._version;
            ...
            {
                if (_version != _dictionary3._version)
                    throw new InvalidOperationException(
                        string.Format("Enumerator version {0} != Dictionary version {1}", _version,
                            _dictionary3._version));

                while (_index < _dictionary3._count)
                {
                    if (_dictionary3._hashCodes[_index] >= 0)
                    {
                        _current =
                            new KeyValuePair<TKey, TValue>(_dictionary3._keys[_index], _dictionary3._values[_index]);
                        _index++;
                        return true;
                    }
                    _index++;
