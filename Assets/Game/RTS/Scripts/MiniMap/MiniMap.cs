...
public class MiniMap : MonoBehaviour
{
    ...
    public void SetSize()
    {
        size = new Rect(localBounds.x / realWorldBounds.x, localBounds.y / realWorldBounds.y,
            localBounds.width / realWorldBounds.width, localBounds.height / realWorldBounds.height);
    ...
    void FixedUpdate()
    {
        ...
        if (details.image)
        {
            ...
            if (moveCamera)
            {
                Vector3 pos = Determine3dLoc(new Vector2(Input.mousePosition.x + cameraLoc.width / 4,
        ...
        for (int x = 0; x < elements.Length; x++)
        {
            ...
            for (int y = 0; y < elements[x].objAmount; y++)
            {
                ...
                if (elements[x].elementMap[y].display)
                {
                    ...
                    GUI.DrawTexture(new Rect(loc.x - size.x, loc.y - size.y, details.size.x, details.size.y),
                        details.image);
                }
            }
        }
    ...
    {
        loc = new Vector2((loc.x - realWorldBounds.x) * size.width + localBounds.x,
    ...
    {
        Rect size = new Rect(realWorldBounds.x / localBounds.x, realWorldBounds.y / localBounds.y,
            realWorldBounds.width / localBounds.width, realWorldBounds.height / localBounds.height);
        Vector3 lLoc = new Vector3((loc.x - localBounds.x) * size.width, 100,
