...
public class GUIManager : MonoBehaviour {
	...
	[HideInInspector]
	public bool mouseOverUnitProduction = false;
	[HideInInspector]
	public int unitProductionIndex = 0;
	[HideInInspector]
	public bool mouseOverTechProduction = false;
	[HideInInspector]
	public int techProductionIndex = 0;
	[HideInInspector]
	public bool mouseOverBuildingProduction = false;
	[HideInInspector]
	public int buildingProductionIndex = 0;
	...
	void OnGUI () {
		...
		// UnitController
		for(int x = 0; x < select.curSelectedLength; x++){
			Rect rectLoc = new Rect((unitGraphic.x+(y*unitBDisp.x))*ratio.x, (unitGraphic.y+(z*unitBDisp.y))*ratio.y, unitGraphic.width*ratio.x, unitGraphic.height*ratio.y);
	...
	public void DisplayUnitProductionDescription (){
		Rect rectLoc = new Rect(unitDescriptionLocation.x*ratio.x, unitDescriptionLocation.y*ratio.y, unitDescriptionLocation.width*ratio.x, unitDescriptionLocation.height*ratio.y);
	...
	public void DisplayTechProductionDescription () {
		Rect rectLoc = new Rect(techDescriptionLocation.x*ratio.x, techDescriptionLocation.y*ratio.y, techDescriptionLocation.width*ratio.x, techDescriptionLocation.height*ratio.y);
		BuildingController sc = select.curBuildSelectedS[0].GetComponent<BuildingController>();
