...
public class Fog : MonoBehaviour {
	...
	void line(int x,int y,int x2, int y2, int radius, ref int[] fogVision, ref Color32[] fColor, float locY, float upwardSlopeHeight, float downwardSlopeHeight) {
		int orPosX = x;
		int orPosY = y;
		int w = x2 - x ;
		int h = y2 - y ;
			...
			if(x < locH.Length){
				...
				if(setting != 0){
					for(int y = 0; y < hiddenRend[x].anchors.Length; y++){
						if(fogVision[(int)(loc.x+hiddenRend[x].anchors[y].x+(loc.y+hiddenRend[x].anchors[y].y)*width)] < setting){
							setting = fogVision[(int)(loc.x+hiddenRend[x].anchors[y].x+(loc.y+hiddenRend[x].anchors[y].y)*width)];
						}
					}
				}
